...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    $mosquename = trim($_POST["mosquename"]);
    $address = trim($_POST["address"]);
    // Prepare an update statement
    $sql = "UPDATE MOSQUE SET MOSQUENAME=?, ADDRESS=? WHERE MOSQUEID=?";
	if($stmt = mysqli_prepare($conn, $sql)){
        // Bind variables to the prepared statement as parameters
        mysqli_stmt_bind_param($stmt, "sss", $mosquename, $address, $mosqueid);
    ...
    if(isset($_GET["mosqueid"]) && !empty(trim($_GET["mosqueid"]))){
        // Get URL parameter
        $mosqueid =  trim($_GET["mosqueid"]);
        
        // Prepare a select statement
        $sql = "SELECT * FROM MOSQUE WHERE MOSQUEID = ?";
        if($stmt = mysqli_prepare($conn, $sql)){

            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "i", $mosqueid);
...
<body>
  ...
  <main id="main" class="main">
    ...
    <div class="pagetitle">
      <h1>Update Mosque Details</h1>
      <nav>
        <ol class="breadcrumb">
          <li class="breadcrumb-item"><a href="index-admin.php">Home</a></li>
          <li class="breadcrumb-item"><a href="admin-mosque-list.php">Back</a></li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
    ...
    <section class="section">
        <div class="card">
            <div class="card-body" action="" method="POST">
              ...
              <p>
                Mosque ID: <input type="text" name="mosqueid" value="<?php echo $mosqueid; ?>" readonly>
              </p>
                <div class="col-md-4">
                    <label for="validationCustom01" class="form-label">Mosque Name</label>
                    <input type="text" class="form-control" name="mosquename" value="<?php echo $mosquename; ?>" requied>
                </div>
                <div class="col-md-4">
