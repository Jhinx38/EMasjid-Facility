          ...
          <div class="card">
            <div class="card-body">
              ...
              if (!isset($_POST['btnSearch'])) {
                $sql = "SELECT BOOKINGID, STATUS_NAME, U_FULLNAME, ITEM_NAME, SDATE, EDATE FROM BOOKING B, USER U, STATUS S, ITEM I WHERE ((B.USERID = U.USERID AND B.STATUS_ID = S.STATUS_ID) AND B.ITEM_ID = I.ITEM_ID) AND MOSQUEID = '$mosqueid'";
                if ($result = mysqli_query($conn, $sql)) {
                  if (mysqli_num_rows($result) > 0) {
                    ...
                    while ($row = mysqli_fetch_array($result)) {
                      ...
                      echo "<td>";
                      echo '<a href="itcomm-update-booking.php?bookingid=' . $row['BOOKINGID'] . '">
                              <i class="bi bi-pen-fill"></i>
                                <span>Update</span>
                              </a>';
                      echo " ";
                      echo '<a href="itcomm-delete-booking.php?bookingid=' . $row['BOOKINGID'] . '">
                ...
                if (empty(trim($_POST['searchData']))) {
                  ...
                  $sql = "SELECT BOOKINGID, STATUS_NAME, U_FULLNAME, ITEM_NAME, SDATE, EDATE FROM BOOKING B, USER U, STATUS S, ITEM I WHERE (((B.USERID = U.USERID AND B.STATUS_ID = S.STATUS_ID) AND B.ITEM_ID = I.ITEM_ID) AND MOSQUEID = '$mosqueid') AND ITEM_NAME LIKE '%$data%'";
                  $result = mysqli_query($conn, $sql);
                  if (mysqli_num_rows($result) > 0) {
                    echo "<table class='table'>";
                    echo "<thead>";
